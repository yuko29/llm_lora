...
from lib.lora_utils import prepare_model_for_lora
import lib.misc_utils as misc_utils
...
# os.environ["WANDB_PROJECT"] = "llama2-7b-retrain"  # name your W&B project
...
class ModelArguments:
    ...
    lora_init: Optional[bool] = field(default=False)
...
def main(return_trainer: bool = False):
    ...
    # wandb.init(resume=must, id=<run_id_you_want_to_resume>)
    ...
    model = prepare_model_for_lora(model=model,
                                    num_ranks=model_args.lora_num_ranks,        # dimension of the updated matrices
                                    lora_alpha=model_args.lora_alpha,           # parameter for scaling
                                    lora_dropout=model_args.lora_dropout,       # dropout probability for layers
                                    target_modules=lora_target_modules,
                                    use_gradient_checkpointing=False,
                                    checkpoint_dir=last_checkpoint,
                                    checkpoint_preprocess_embedding=False)
    ...
    if data_args.dataset_name in ["c4"]:
        eval_dataset_dict = {
            "c4": eval_dataset,
    ...
    ################################################################################################################
    # batch_size = 64
    training_args.gradient_accumulation_steps = model_args.batch_size // training_args.per_device_train_batch_size
    ...
    if training_args.do_eval:
        ...
        if isinstance(trainer.eval_dataset, dict):
            metrics = {}
            for _eval_dataset_name, _eval_dataset in trainer.eval_dataset.items():
                _metrics = trainer.evaluate(
                    eval_dataset=_eval_dataset,
                    metric_key_prefix=f"eval_{_eval_dataset_name}")
                _evaluation_post_processing(
                    prefix=f"{_eval_dataset_name}_",
                    metrics=_metrics,
                    eval_dataset=_eval_dataset)
        ...
        # MMLU evaluation will overwrite some of the attributes
        # of the `tokenizer` and `trainer.args`. After this call
        # we should no longer use the `trainer`.
        # click.secho("MMLU Evaluation", fg="blue")
        # if not all([
        #     trainer.model is model,
        #     trainer.tokenizer is tokenizer]):
        #     raise ValueError
        # mmlu_metrics = mmlu_utils.run_evaluation(trainer=trainer)
        # trainer.log(mmlu_metrics)
